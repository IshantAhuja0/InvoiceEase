...
<motion.div
  key={template.id}
  className="group bg-gray-200 rounded-2xl overflow-hidden shadow-lg cursor-pointer flex flex-col"
  variants={cardVariants}
  whileHover={{ scale: 1.02 }}
  transition={{ type:"inertia", stiffness: 120 }}
>
  ...
  <div className="overflow-hidden flex-1">
    <motion.img
      src={template.src}
      alt={template.name}
      className="w-full h-52 group-hover:h-78 object-cover transition-all duration-500 ease-in-out"
    />
  </div>

  {/* Footer hides on hover */}
  <div className="p-2 flex flex-col justify-between group-hover:hidden">
    <h2 className="text-lg font-semibold m-2">{template.name}</h2>
    <button className="bg-blue-950 text-white px-4 py-2 rounded-md hover:bg-blue-900 transition mt-auto flex items-center gap-1 justify-center">
      Create Invoice
